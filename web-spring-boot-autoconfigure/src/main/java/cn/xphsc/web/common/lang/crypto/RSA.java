...
import cn.xphsc.web.common.lang.crypto.enums.RSASignature;
...
import static cn.xphsc.web.common.lang.crypto.Keys.getKey;
...
public class RSA {
    ...
    static {
        try {
            RSA_KEY_FACTORY = KeyFactory.getInstance(CryptoConstant.RSA);
        } catch (Exception e) {
    ...
    public static String encrypt(String s, String publicKey) {
        byte[] bytes = encrypt(ByteUtils.toBytes(s), getPublicKey(publicKey));
        if (bytes != null) {
    ...
    public static String encrypt(String s, PublicKey publicKey) {
        byte[] bytes = encrypt(ByteUtils.toBytes(s), publicKey);
        if (bytes != null) {
            return new String(bytes);
        } else {
            return null;
    ...
    public static String decrypt(String s, String privateKey) {
        byte[] bytes = decrypt(ByteUtils.toBytes(s), getPrivateKey(privateKey));
        if (bytes != null) {
            return new String(bytes);
        }
        return null;
    ...
    public static String decrypt(String s, PrivateKey privateKey) {
        byte[] bytes = decrypt(ByteUtils.toBytes(s), privateKey);
        if (bytes != null) {
            return new String(bytes);
        }
        return null;
    ...
    public static String sign(String s, String privateKey) {
        byte[] bytes = sign(ByteUtils.toBytes(s), getPrivateKey(privateKey), RSASignature.MD5);
        if (bytes != null) {
            return new String(bytes);
        }
        return null;
    ...
    public static String sign(String s, PrivateKey privateKey) {
        byte[] bytes = sign(ByteUtils.toBytes(s), privateKey, RSASignature.MD5);
        if (bytes != null) {
            return new String(bytes);
        }
        return null;
    ...
    public static String sign(String s, String privateKey, RSASignature signModel) {
        byte[] bytes = sign(ByteUtils.toBytes(s), getPrivateKey(privateKey), signModel);
        if (bytes != null) {
            return new String(bytes);
        }
        return null;
    ...
    public static String sign(String s, PrivateKey privateKey, RSASignature signModel) {
        byte[] bytes = sign(ByteUtils.toBytes(s), privateKey, signModel);
        if (bytes != null) {
            return new String(bytes);
        }
        return null;
    ...
    public static boolean verify(String s, String publicKey, String sign) {
        return verify(ByteUtils.toBytes(s), getPublicKey(publicKey), ByteUtils.toBytes(sign), RSASignature.MD5);
    }

    public static boolean verify(String s, PublicKey publicKey, String sign) {
        return verify(ByteUtils.toBytes(s), publicKey, ByteUtils.toBytes(sign), RSASignature.MD5);
    }
    ...
    public static boolean verify(String s, String publicKey, String sign, RSASignature signModel) {
        return verify(ByteUtils.toBytes(s), getPublicKey(publicKey), ByteUtils.toBytes(sign), signModel);
    }

    public static boolean verify(String s, PublicKey publicKey, String sign, RSASignature signModel) {
        return verify(ByteUtils.toBytes(s), publicKey, ByteUtils.toBytes(sign), signModel);
    }
    ...
    public static RSAPrivateKey getPrivateKey(File file) {
        byte[] bytes = ByteUtils.bytes(getKey(file), StandardCharsets.ISO_8859_1);
        return getPrivateKey(bytes);
    }

    public static RSAPrivateKey getPrivateKey(String key) {
        byte[] bytes = ByteUtils.bytes(key, StandardCharsets.ISO_8859_1);
        return getPrivateKey(bytes);
    }
    ...
    public static Pair<RSAPublicKey, RSAPrivateKey> generatorKeys() {
        return generatorKeys(CryptoConstant.RSA_KEY_LENGTH);
    }
    ...
    public static Pair<RSAPublicKey, RSAPrivateKey> generatorKeys(int length) {
        try {
            KeyPairGenerator keyPairGen = KeyPairGenerator.getInstance(CryptoConstant.RSA);
