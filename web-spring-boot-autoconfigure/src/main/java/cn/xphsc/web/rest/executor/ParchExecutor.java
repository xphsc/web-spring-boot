...
public class ParchExecutor<T> extends AbstractExecutor<T>{
    ...
    public ParchExecutor(RestTemplate restTemplate, RestEntity entity) {
        super(restTemplate);
        this.entity = entity;
    ...
    protected T doExecute() {
        if (clazzResponseType != null) {
            if (uriVariable) {
                if (uriVariables != null && arrayUriVariables == null) {
                    if (requestEntity != null) {
                        return (T)exchange(entity.url(), HttpMethod.PATCH, requestEntity, clazzResponseType, uriVariables);
                    }else{
                        return (T)exchange(entity.url(), HttpMethod.PATCH, HttpEntity.EMPTY, clazzResponseType, uriVariables);
                    }
                } else {
                    if (requestEntity!= null) {
                        return (T)exchange(entity.url(), HttpMethod.PATCH, requestEntity, clazzResponseType, arrayUriVariables);
                    } else {
                        return (T)exchange(entity.url(), HttpMethod.PATCH, HttpEntity.EMPTY, clazzResponseType, arrayUriVariables);
                    }
                }
            }
        }else{
            if (uriVariable&& requestEntity != null) {
                if (uriVariables != null && arrayUriVariables == null) {
                    this.booleanMethod(entity.url(), requestEntity, HttpMethod.PATCH, uriVariables);
                }else{
                    this.booleanMethod(entity.url(), requestEntity, HttpMethod.PATCH, arrayUriVariables);
                }
            }
